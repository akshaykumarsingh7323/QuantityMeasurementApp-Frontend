...
export class MeasurementComponent implements OnInit {
  ...
  private doComparison(): void {
    ...
    this.measurementService.compare(body).subscribe({
      next: (res: any) => {
        this.loading = false;
        const eq = String(res.resultString) === 'true';

        // Normalize values to determine < or > safely
        const v1 = this.normalize(this.valFrom, this.unitFrom);
        const v2 = this.normalize(this.valTo, this.unitTo);

        const sym = eq ? '=' : (v1 > v2 ? '>' : '<');
        this.resultValue = `${this.valFrom} ${this.unitFrom} ${sym} ${this.valTo} ${this.unitTo}`;
        
        // Ensure result text makes sense
        this.resultNote = eq ? 'Both values are equal.' : 'Values are not equal.';
